# PyTorch training and evaluation utilities for word embedding genre classification
...
def train_model(model, x_train, y_train, x_val, y_val,
                num_epochs=20, batch_size=32, learning_rate=1e-3, verbose=True):
    """Train the model; returns a history dict with loss/accuracy keys."""
    tX = torch.tensor(x_train, dtype=torch.long)
    tY = torch.tensor(y_train, dtype=torch.long)
    vX = torch.tensor(x_val, dtype=torch.long)
...
def evaluate_model(model, x_test, y_test):
    ...
    model.eval()
    with torch.no_grad():
        xt = torch.tensor(x_test, dtype=torch.long)
